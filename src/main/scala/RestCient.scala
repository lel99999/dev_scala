...
Command-line REST client: building a working example 53 println(responseBody)
}
def handleGetRequest = {
  ...
  val responseBody =
new DefaultHttpClient().execute(httpget, new BasicResponseHandler()) println(responseBody)
}
